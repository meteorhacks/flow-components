FlowComponents = {};
// notify to avoid promises
FlowComponents._avoidPromises = new Meteor.EnvironmentVariable();
...
FlowComponents.callAction = function callAction(action) {
  ...
  return this.applyAction(action, args);
};

FlowComponents.applyAction = function callAction(action, args) {
  var currentView = Blaze.currentView;
  ...
  return component.emitPrivateAction(action, args);
};
