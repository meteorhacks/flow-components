...
EchoComponents.define = function(name, options) {
  options = options || {};
...
Template.render = new Template("Template.render", function() {
  ...
  if(!componentName) {
    throw new Error("EchoComponent needs 'component' parameter");
  }

  if(!ComponentsStore[componentName]) {
    throw new Error("No such EchoComponent: " + componentName);
  }
...
EchoComponents.renderLayout = function(layout, regions) {
  var rootDomNode = $('#___echo').get(0);
